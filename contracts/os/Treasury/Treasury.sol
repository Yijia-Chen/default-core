...
contract def_Treasury is DefaultOSModule {
    ...
    function openVault(address token_, uint8 fee_) external onlyOS {
        ...
        require(address(treasuryVaults[token_].vault) == address(0), "def_Treasury | openVault(): vault already exists");
        require (fee_ >= 0 && fee_ <= 100, "defTreasury | openVault(): fee must be 0 <= fee <= 100");

        // naming standard for the vault share tokens
        IERC20Metadata _AssetData = IERC20Metadata(token_);
        string memory vaultName = string(abi.encodePacked("Default Treasury Vault: ", _AssetData.symbol()));
        string memory vaultSymbol = string(abi.encodePacked(_AssetData.symbol(), "-VS"));
        uint8 vaultDecimals = _AssetData.decimals();

        // create the token contract for this vault
        Vault newVault = new Vault(token_, vaultName, vaultSymbol, vaultDecimals);

        // save it to the registry
        treasuryVaults[token_] = TreasuryVault(fee_, newVault);

        // record event for frontend
        emit VaultOpened(newVault, _OS.currentEpoch());
    }
    ...
    function withdrawFromVault(address token_, uint256 amountshares_) external onlyOS {

        // find the treasury vault for the given token
        TreasuryVault memory tsyVault = treasuryVaults[token_];
        require (address(tsyVault.vault) != address(0), "def_Treasury | deposit(): vault does not exist for token");
        
        // withdraw from the vault to the OS
        tsyVault.vault.withdraw(address(_OS), amountshares_);

        emit Deposited(tsyVault.vault, address(this), amountshares_, _OS.currentEpoch());
    }
    ...
    function changeFee(address token_, uint8 newFeePctg) external onlyOS {
        ...
        emit VaultFeeChanged(tsyVault.vault, newFeePctg, _OS.currentEpoch());
    }
    ...
    function deposit(address token_, uint256 amountTokens_) external {
        ...
        emit Deposited(tsyVault.vault, msg.sender, amountTokens_, _OS.currentEpoch());
    }
    ...
    function withdraw(address token_, uint256 amountShares_) external {
        ...
        tsyVault.vault.transferFrom(msg.sender, address(_OS), withdrawFeeCollected);

        // use subtraction to avoid rounding errors
        uint256 amountWithdrawn = tsyVault.vault.withdraw(msg.sender, amountShares_ - withdrawFeeCollected);

        emit Withdrawn(tsyVault.vault, msg.sender, amountWithdrawn, _OS.currentEpoch());
    }
}