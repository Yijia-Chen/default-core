...
describe("Mining.sol", function () {
  before(async function () {
    ...
    this.TreasuryInstaller = await ethers.getContractFactory(
      "def_TreasuryInstaller"
    );
    this.treasuryModule = await this.TreasuryInstaller.deploy();
    await this.treasuryModule.deployed();

    this.MiningInstaller = await ethers.getContractFactory(
      "def_MiningInstaller"
    );
  ...
  beforeEach(async function () {
    ...
    this.token = await ethers.getContractAt(
      "def_Token",
      await this.default.getModule("0x544b4e")
    );
    ...
    this.epoch = await ethers.getContractAt(
      "def_Epoch",
      await this.default.getModule("0x455043")
    );

    await this.default.installModule(this.treasuryModule.address);
    this.treasury = await ethers.getContractAt(
      "def_Treasury",
      await this.default.getModule("0x545359")
    );

    await this.default.installModule(this.miningModule.address);
    this.mining = await ethers.getContractAt(
      "def_Mining",
      await this.default.getModule("0x4d4e47")
    ); //MNG
  ...
  describe("assignVault", async function () {
    it("allows only owner", async function () {
      ...
      await expect(mining.assignVault(this.lpToken.address)).to.be.revertedWith(
    ...
    it("does not allow being set more than once", async function () {
      ...
      await expect(
        this.mining.assignVault(this.lpToken.address)
      ).to.be.revertedWith("can only assign vault once");
    });
  });
  ...
  describe("setTokenBonus", async function () {
    ...
    it("allows only owner", async function () {
      const mining = await this.mining.connect(this.userA);
      await expect(mining.setTokenBonus(1)).to.be.revertedWith(
        "only the os owner can make this call"
      );
    });
  });
  ...
  describe("issueRewards", async function () {
    ...
    it("successfully sets new accumulated rewards per share", async function () {
      ...
      expect((await this.mining.accRewardsPerShare()).toNumber()).to.equal(
        10 * MULT
      );
      ...
      await this.lpToken
        .connect(this.userA)
        .approve(this.vault.address, 200000);
      await this.treasury
        .connect(this.userA)
        .deposit(this.vault.address, 200000);
    ...
    it("successfully mints token bonus", async function () {
      ...
      expect(
        (await this.token.balanceOf(this.dev.address)).toNumber()
      ).to.equal(5000);
    });
    ...
    it("disallows duplicate issuance in the same epoch", async function () {
      ...
      await expect(this.mining.issueRewards()).to.be.revertedWith(
        "rewards have already been accumulated for the current epoch"
      );
    });
  ...
  describe("pendingRewards", async function () {
    ...
    it("successfully calculates rewards", async function () {
      ...
      await this.lpToken
        .connect(this.userA)
        .approve(this.vault.address, 150000);
      await this.treasury
        .connect(this.userA)
        .deposit(this.vault.address, 150000);
